...
import vn.edu.tdc.ltdd2.adapters.HistoryOrderAdapter;
import vn.edu.tdc.ltdd2.data_models.Order;
import vn.edu.tdc.ltdd2.data_models.ReportOrder;
import vn.edu.tdc.ltdd2.data_models.Status;
...
public class OrderHistoryActivity extends AppCompatActivity implements SearchView.OnQueryTextListener, NavigationView.OnNavigationItemSelectedListener {
    ...
    Intent intent;
    String accountID = "", username = "", role = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Picker date to
        tvHistoryCartPickerTo.setOnClickListener(view -> showDateDialog(tvHistoryCartPickerTo));
        //Picker date from
        tvHistoryCartPickerFrom.setOnClickListener(view -> showDateDialog(tvHistoryCartPickerFrom));
        //Filter by time
        btnHistoryCartSubmit.setOnClickListener(view -> {
            ...
            Date dateOrder;
            try {
                dateTo = simpleDateFormat.parse(tvHistoryCartPickerTo.getText().toString(), new ParsePosition(0));
                dateFrom = simpleDateFormat.parse(tvHistoryCartPickerFrom.getText().toString(), new ParsePosition(0));
    ...
    private void loadDataOrder() {
        ...
        queryByOrder.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    Order order;
                    String keyOrder, accountID, address, created_at, name, note, phone;
                    int status, total;
                    for (DataSnapshot o : snapshot.getChildren()) {
                        ...
                        order = new Order(keyOrder, accountID, address, created_at, note, status, total, phone, name);
    ...
    private void setControl() {
        ...
        subtitleAppbar.setText(R.string.titleLSDH);
    ...
    private void styleCell() {
        ...
        cellStyleTitleMain.setAlignment(HorizontalAlignment.CENTER);
        cellStyleTitleMain.setFillForegroundColor(HSSFColor.YELLOW.index);
        cellStyleTitleMain.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        ...
        cellStyleTitleResult.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        cellStyleTitleResult.setAlignment(HorizontalAlignment.CENTER);
        ...
        cellStyleValueResult.setFillPattern(FillPatternType.SOLID_FOREGROUND);
    }
    ...
    private void setCellDataTitle() {
        ...
        setCellTitle("Địa chỉ", 2);
        setCellTitle("Ghi chú", 3);
        setCellTitle("Trạng thái đơn hàng", 4);
        setCellTitle("Tổng hoá đơn", 5);
    }
    ...
    private void setRowDataValue() {
        ...
        for (int i = 0; i < arrOrderFilter.size(); i++) {
            ...
            setCellValue(arrOrderFilter.get(i).getAddress(), 2);
            setCellValue(arrOrderFilter.get(i).getNote(), 3);
            setCellValue(valueStatus, 4);
            setCellValue(formatPrice(arrOrderFilter.get(i).getTotal()), 5);
        }
    }
